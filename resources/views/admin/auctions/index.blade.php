...
<x-admin-layout>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="overflow-hidden p-4 sm:p-8 bg-white shadow sm:rounded-lg">
                <table class="w-full">
                    <thead>
                    <tr>
                        <th class="text-left p-4 bg-indigo-50">Müzayede Adı</th>
                        <th class="text-left p-4 bg-indigo-50">Başlangıç Tarihi</th>
                        <th class="text-left p-4 bg-indigo-50">Bitiş Tarihi</th>
                        <th class="text-left p-4 bg-indigo-50">Bitişe Kalan Süre</th>
                        <th class="text-left p-4 bg-indigo-50">Düzenle</th>
                        <th class="text-left p-4 bg-indigo-50">Sil</th>
                    </tr>
                    </thead>
                    <tbody>
                    @foreach($auctions as $auction)
                        <tr>
                            <td class="text-left font-bold p-4 border-b">{{$auction->name}}</td>
                            <td class="text-left font-bold p-4 border-b">{{$auction->start_date}}</td>
                            <td class="text-left font-bold p-4 border-b">{{$auction->end_date}}</td>
                            <td class="text-left font-bold p-4 border-b">
                                @if(Carbon::now()->greaterThan(Carbon::parse($auction->end_date)))
                                    <span class="text-red-600">Müzayede Bitti</span>
                                @else
                                    <span class="bg-gray-100 text-gray-800 text-xs font-medium inline-flex items-center px-2.5 py-0.5 rounded mr-2 dark:bg-gray-700 dark:text-white border border-gray-500">
  ...
  {{Carbon::now()->diffAsCarbonInterval($auction->end_date)}}
                            ...
                            <td class="text-left text-center font-bold p-4 border-b">
